...
def start_stop(json_val, context):
	...
	# Convert 'instances' in json to python 'list'. Remove unwanted spaces in the list. 
        if 'instanceNames' in json_val:
            instances=map(str.strip,str(json_val['instanceNames']).split(","))
	    type="NameTag"
	elif 'instanceIds' in json_val:
	    instances=map(str.strip,str(json_val['instanceIds']).split(","))
	    type="InstanceId"
